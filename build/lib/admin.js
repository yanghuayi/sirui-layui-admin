...
layui.define("view", function(e) {
	var a = layui.jquery,
		...
		x = {
			...
			sideFlexible: function(e) {
				var i = u,
					t = a("#" + m),
					s = x.screen();
				"spread" === e ? (t.removeClass(v).addClass(p), s < 2 ? i.addClass(f) : i.removeClass(f), i.removeClass(b)) : (t.removeClass(p).addClass(v), s < 2 ? i.removeClass(b) : i.addClass(b), i.removeClass(f)), layui.event.call(this, n.MOD_NAME, "side({*})", {
					status: e
				})
			},
			removeTabs (obj) {
		...
		C = x.events = {
			...
			refresh: function() {
				var page = a('.layadmin-tabsbody-item.layui-show');
				if (page.prop('tagName') === 'IFRAME') {
					page[0].contentWindow.location.reload();
				} else {
					layui.index.render()
				}
			},
			...
			setTheme: function(e) {
				...
				e.hasClass(c) || (e.addClass(c).siblings(".layui-this").removeClass(c), a.color[i] && (a.color[i].index = i, x.theme({
					color: a.color[i]
				})))
			},
	...
	function() {
		...
		e.theme && x.theme(e.theme), o.addClass("layui-layout-body"), x.screen() < 1 && !n.mobileTabs && delete n.pageTabs, n.pageTabs || u.addClass("layadmin-tabspage-none"), l.ie && l.ie < 10 && s.error("IE" + l.ie + "下访问可能不佳，推荐使用：Chrome / Firefox / Edge 等高级浏览器", {
			offset: "auto",
			id: "LAY_errorIE"
		})
	...
	o.on("click", z, function() {
		...
		e.index && x.removeTabs(x.tabsBody(e.index)), k(i), x.delResize()
